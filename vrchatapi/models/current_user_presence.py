...
class CurrentUserPresence(BaseModel):
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "avatarImageUrl" not in obj and "avatar_image_url" in obj:
            obj["avatarImageUrl"] = obj["avatar_image_url"]
        obj.pop("avatar_image_url", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("avatarImageUrl" if serialize else "avatar_image_url"): _to_legacy_value(getattr(self, "avatar_image_url", None), serialize),
            ("avatarThumbnail" if serialize else "avatar_thumbnail"): _to_legacy_value(getattr(self, "avatar_thumbnail", None), serialize),
            ("banner" if serialize else "banner"): _to_legacy_value(getattr(self, "banner", None), serialize),
