...
class CalendarEventDiscovery(BaseModel):
    """
    ...
    """ # noqa: E501
    next_cursor: Optional[StrictStr] = Field(default=None, validation_alias=AliasChoices("nextCursor", "next_cursor"), serialization_alias="nextCursor", description="Base64-encoded JSON:   type: object   properties:     dataSource:       type: string       enum:         - featured         - personalized     dataIndex:       type: integer       format: int32     phase:       type: string       enum:         - all         - live         - upcoming       description: see CalendarEventDiscoveryScope     asOf:       type: integer       format: int64       description: milliseconds since Unix epoch     paramHash:       type: string       format: string       description: Base64-encoded 256-bit hash of the original query parameters")
    results: Optional[List[CalendarEvent]] = None
    ...
    __properties: ClassVar[List[str]] = ["nextCursor", "results"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "nextCursor" not in obj and "next_cursor" in obj:
            obj["nextCursor"] = obj["next_cursor"]
        obj.pop("next_cursor", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("nextCursor" if serialize else "next_cursor"): _to_legacy_value(getattr(self, "next_cursor", None), serialize),
