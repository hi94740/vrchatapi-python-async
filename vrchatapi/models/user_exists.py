...
class UserExists(BaseModel):
    ...
    """ # noqa: E501
    name_ok: Optional[StrictBool] = Field(default=False, validation_alias=AliasChoices("nameOk", "name_ok"), serialization_alias="nameOk", description="Is the username valid?")
    user_exists: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("userExists", "user_exists"), serialization_alias="userExists", description="Status if a user exist with that username or userId.")
    ...
    __properties: ClassVar[List[str]] = ["nameOk", "userExists"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "nameOk" not in obj and "name_ok" in obj:
            obj["nameOk"] = obj["name_ok"]
        obj.pop("name_ok", None)
        if "userExists" not in obj and "user_exists" in obj:
            obj["userExists"] = obj["user_exists"]
        obj.pop("user_exists", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("nameOk" if serialize else "name_ok"): _to_legacy_value(getattr(self, "name_ok", None), serialize),
            ("userExists" if serialize else "user_exists"): _to_legacy_value(getattr(self, "user_exists", None), serialize),
        }
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "nameOk": obj.get("nameOk") if obj.get("nameOk") is not None else False,
            "userExists": obj.get("userExists") if obj.get("userExists") is not None else False
        })
        return _obj
