...
class GetUserGroupInstances200Response(BaseModel):
    """
    ...
    """ # noqa: E501
    fetched_at: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("fetchedAt", "fetched_at"), serialization_alias="fetchedAt")
    ...
    __properties: ClassVar[List[str]] = ["fetchedAt", "instances"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "fetchedAt" not in obj and "fetched_at" in obj:
            obj["fetchedAt"] = obj["fetched_at"]
        obj.pop("fetched_at", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("fetchedAt" if serialize else "fetched_at"): _to_legacy_value(getattr(self, "fetched_at", None), serialize),
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.instances:
            for _item_instances in self.instances:
                if _item_instances:
                    _items.append(_to_openapi_value(_item_instances))
