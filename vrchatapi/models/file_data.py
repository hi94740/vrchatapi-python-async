...
class FileData(BaseModel):
    ...
    file_name: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, validation_alias=AliasChoices("fileName", "file_name"), serialization_alias="fileName")
    md5: Optional[Annotated[str, Field(min_length=0, strict=True)]] = None
    size_in_bytes: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, validation_alias=AliasChoices("sizeInBytes", "size_in_bytes"), serialization_alias="sizeInBytes")
    status: Optional[FileStatus] = None
    upload_id: Optional[Annotated[str, Field(min_length=0, strict=True)]] = Field(default=None, validation_alias=AliasChoices("uploadId", "upload_id"), serialization_alias="uploadId")
    ...
    __properties: ClassVar[List[str]] = ["category", "fileName", "md5", "sizeInBytes", "status", "uploadId", "url"]

    @field_validator('category')
    def category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['multipart', 'queued', 'simple']):
            raise ValueError("must be one of enum values ('multipart', 'queued', 'simple')")
        return value
    ...
    def __preprocess_input_names(
        ...
        if "sizeInBytes" not in obj and "size_in_bytes" in obj:
            obj["sizeInBytes"] = obj["size_in_bytes"]
        obj.pop("size_in_bytes", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("md5" if serialize else "md5"): _to_legacy_value(getattr(self, "md5", None), serialize),
            ("sizeInBytes" if serialize else "size_in_bytes"): _to_legacy_value(getattr(self, "size_in_bytes", None), serialize),
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "category": obj.get("category") if obj.get("category") is not None else 'queued',
            "fileName": obj.get("fileName"),
            "md5": obj.get("md5"),
            "sizeInBytes": obj.get("sizeInBytes"),
            "status": obj.get("status") if obj.get("status") is not None else FileStatus.WAITING,
            "uploadId": obj.get("uploadId") if obj.get("uploadId") is not None else '',
            "url": obj.get("url")
        })
        return _obj
