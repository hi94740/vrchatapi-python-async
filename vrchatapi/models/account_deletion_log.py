...
class AccountDeletionLog(BaseModel):
    ...
    """ # noqa: E501
    date_time: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("dateTime", "date_time"), serialization_alias="dateTime", description="Date and time of the deletion request.")
    deletion_scheduled: Optional[datetime] = Field(default=None, validation_alias=AliasChoices("deletionScheduled", "deletion_scheduled"), serialization_alias="deletionScheduled", description="When the deletion is scheduled to happen, standard is 14 days after the request.")
    message: Optional[StrictStr] = Field(default='Deletion requested', description="Typically \"Deletion requested\" or \"Deletion canceled\". Other messages like \"Deletion completed\" may exist, but are these are not possible to see as a regular user.", json_schema_extra={"examples": ["Deletion requested"]})
    ...
    __properties: ClassVar[List[str]] = ["dateTime", "deletionScheduled", "message"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "dateTime" not in obj and "date_time" in obj:
            obj["dateTime"] = obj["date_time"]
        obj.pop("date_time", None)
        if "deletionScheduled" not in obj and "deletion_scheduled" in obj:
            obj["deletionScheduled"] = obj["deletion_scheduled"]
        obj.pop("deletion_scheduled", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("dateTime" if serialize else "date_time"): _to_legacy_value(getattr(self, "date_time", None), serialize),
            ("deletionScheduled" if serialize else "deletion_scheduled"): _to_legacy_value(getattr(self, "deletion_scheduled", None), serialize),
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if deletion_scheduled (nullable) is None
        # and model_fields_set contains the field
        if self.deletion_scheduled is None and "deletion_scheduled" in self.model_fields_set:
            _dict['deletionScheduled'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "deletionScheduled": obj.get("deletionScheduled"),
            "message": obj.get("message") if obj.get("message") is not None else 'Deletion requested'
        })
        return _obj
