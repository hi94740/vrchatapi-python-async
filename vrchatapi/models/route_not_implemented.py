...
class RouteNotImplemented(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["error", "status_code"]

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['The endpoint you\'re looking for is not implemented by our system.']):
            raise ValueError("must be one of enum values ('The endpoint you\'re looking for is not implemented by our system.')")
        return value
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            ("status_code" if serialize else "status_code"): _to_legacy_value(getattr(self, "status_code", None), serialize),
        }
