...
class TwoFactorRecoveryCodes(BaseModel):
    ...
    """ # noqa: E501
    otp: Optional[List[TwoFactorRecoveryCodesOtpInner]] = None
    requires_two_factor_auth: Optional[List[StrictStr]] = Field(default=None, validation_alias=AliasChoices("requiresTwoFactorAuth", "requires_two_factor_auth"), serialization_alias="requiresTwoFactorAuth")
    ...
    __properties: ClassVar[List[str]] = ["otp", "requiresTwoFactorAuth"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "requiresTwoFactorAuth" not in obj and "requires_two_factor_auth" in obj:
            obj["requiresTwoFactorAuth"] = obj["requires_two_factor_auth"]
        obj.pop("requires_two_factor_auth", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("otp" if serialize else "otp"): _to_legacy_value(getattr(self, "otp", None), serialize),
            ("requiresTwoFactorAuth" if serialize else "requires_two_factor_auth"): _to_legacy_value(getattr(self, "requires_two_factor_auth", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in otp (list)
        _items = []
        if self.otp:
            for _item_otp in self.otp:
                if _item_otp:
                    _items.append(_to_openapi_value(_item_otp))
            _dict['otp'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "otp": [TwoFactorRecoveryCodesOtpInner.from_dict(_item) for _item in obj["otp"]] if obj.get("otp") is not None else None,
            "requiresTwoFactorAuth": obj.get("requiresTwoFactorAuth")
        })
        return _obj
