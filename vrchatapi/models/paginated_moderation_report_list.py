...
class PaginatedModerationReportList(BaseModel):
    """
    ...
    """ # noqa: E501
    has_next: Optional[StrictBool] = Field(default=None, validation_alias=AliasChoices("hasNext", "has_next"), serialization_alias="hasNext", description="Whether there are more results after this page.")
    results: Optional[List[ModerationReport]] = Field(default=None, description="The list of moderation reports.")
    total_count: Optional[StrictInt] = Field(default=None, validation_alias=AliasChoices("totalCount", "total_count"), serialization_alias="totalCount", description="The total number of results that the query would return if there were no pagination.")
    ...
    __properties: ClassVar[List[str]] = ["hasNext", "results", "totalCount"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "hasNext" not in obj and "has_next" in obj:
            obj["hasNext"] = obj["has_next"]
        obj.pop("has_next", None)
        if "totalCount" not in obj and "total_count" in obj:
            obj["totalCount"] = obj["total_count"]
        obj.pop("total_count", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("hasNext" if serialize else "has_next"): _to_legacy_value(getattr(self, "has_next", None), serialize),
            ("results" if serialize else "results"): _to_legacy_value(getattr(self, "results", None), serialize),
            ("totalCount" if serialize else "total_count"): _to_legacy_value(getattr(self, "total_count", None), serialize),
        }
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "results": [ModerationReport.from_dict(_item) for _item in obj["results"]] if obj.get("results") is not None else None,
