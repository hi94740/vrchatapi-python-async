...
class CalendarEventRecurrence(BaseModel):
    ...
    """ # noqa: E501
    days_of_week: Optional[List[CalendarDayOfWeek]] = Field(default=None, validation_alias=AliasChoices("daysOfWeek", "days_of_week"), serialization_alias="daysOfWeek", description="Which days of the week the event will be scheduled, only valid/present for \"weekly\" recurring events")
    end: Optional[CalendarEventRecurrenceEnd] = None
    frequency: Optional[CalendarEventFrequency] = None
    interval: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="How often the event will be scheduled, in units of \"frequency\"")
    timezone: Optional[StrictStr] = Field(default=None, description="The timezone the event will be scheduled in, in Area/Location format", json_schema_extra={"examples": ["America/Chicago"]})
    ...
    __properties: ClassVar[List[str]] = ["daysOfWeek", "end", "frequency", "interval", "timezone"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "daysOfWeek" not in obj and "days_of_week" in obj:
            obj["daysOfWeek"] = obj["days_of_week"]
        obj.pop("days_of_week", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("daysOfWeek" if serialize else "days_of_week"): _to_legacy_value(getattr(self, "days_of_week", None), serialize),
            ("end" if serialize else "end"): _to_legacy_value(getattr(self, "end", None), serialize),
            ("frequency" if serialize else "frequency"): _to_legacy_value(getattr(self, "frequency", None), serialize),
            ("interval" if serialize else "interval"): _to_legacy_value(getattr(self, "interval", None), serialize),
            ("timezone" if serialize else "timezone"): _to_legacy_value(getattr(self, "timezone", None), serialize),
        }
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.end:
            _dict['end'] = _to_openapi_value(self.end)
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "end": CalendarEventRecurrenceEnd.from_dict(obj["end"]) if obj.get("end") is not None else None,
            "frequency": obj.get("frequency") if obj.get("frequency") is not None else CalendarEventFrequency.WEEKLY,
