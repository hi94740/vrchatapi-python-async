...
class FileVersionUploadStatus(BaseModel):
    """
    ...
    """ # noqa: E501
    etags: Optional[Annotated[List[Dict[str, Any]], Field(min_length=0)]] = Field(default=None, description="Unknown")
    file_name: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, validation_alias=AliasChoices("fileName", "file_name"), serialization_alias="fileName", json_schema_extra={"examples": ["Avatar-MyAvatar-Un.file_00000000-0000-0000-0000-000000000000.1.unitypackage"]})
    max_parts: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, validation_alias=AliasChoices("maxParts", "max_parts"), serialization_alias="maxParts", json_schema_extra={"examples": [1000]})
    next_part_number: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, validation_alias=AliasChoices("nextPartNumber", "next_part_number"), serialization_alias="nextPartNumber", json_schema_extra={"examples": [0]})
    parts: Optional[Annotated[List[Dict[str, Any]], Field(min_length=0)]] = None
    upload_id: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, validation_alias=AliasChoices("uploadId", "upload_id"), serialization_alias="uploadId", json_schema_extra={"examples": ["xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx_xxxxxxxxxxxxxxxxxxxxxxxx.xxxxxxx_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx..xxxxxxxxxxxxxxxxxxxxxxx"]})
    ...
    __properties: ClassVar[List[str]] = ["etags", "fileName", "maxParts", "nextPartNumber", "parts", "uploadId"]
    ...
    def __preprocess_input_names(
        ...
        if "maxParts" not in obj and "max_parts" in obj:
            obj["maxParts"] = obj["max_parts"]
        obj.pop("max_parts", None)
        if "nextPartNumber" not in obj and "next_part_number" in obj:
            obj["nextPartNumber"] = obj["next_part_number"]
        obj.pop("next_part_number", None)
        if "uploadId" not in obj and "upload_id" in obj:
            obj["uploadId"] = obj["upload_id"]
        obj.pop("upload_id", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("etags" if serialize else "etags"): _to_legacy_value(getattr(self, "etags", None), serialize),
            ("fileName" if serialize else "file_name"): _to_legacy_value(getattr(self, "file_name", None), serialize),
            ("maxParts" if serialize else "max_parts"): _to_legacy_value(getattr(self, "max_parts", None), serialize),
            ("nextPartNumber" if serialize else "next_part_number"): _to_legacy_value(getattr(self, "next_part_number", None), serialize),
            ("parts" if serialize else "parts"): _to_legacy_value(getattr(self, "parts", None), serialize),
            ("uploadId" if serialize else "upload_id"): _to_legacy_value(getattr(self, "upload_id", None), serialize),
        }
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "maxParts": obj.get("maxParts"),
            "nextPartNumber": obj.get("nextPartNumber"),
            "parts": obj.get("parts"),
