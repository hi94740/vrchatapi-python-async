...
class APIHealth(BaseModel):
    ...
    """ # noqa: E501
    build_version_tag: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, validation_alias=AliasChoices("buildVersionTag", "build_version_tag"), serialization_alias="buildVersionTag")
    ok: Optional[StrictBool] = None
    server_name: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, validation_alias=AliasChoices("serverName", "server_name"), serialization_alias="serverName")
    ...
    __properties: ClassVar[List[str]] = ["buildVersionTag", "ok", "serverName"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "buildVersionTag" not in obj and "build_version_tag" in obj:
            obj["buildVersionTag"] = obj["build_version_tag"]
        obj.pop("build_version_tag", None)
        if "serverName" not in obj and "server_name" in obj:
            obj["serverName"] = obj["server_name"]
        obj.pop("server_name", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("buildVersionTag" if serialize else "build_version_tag"): _to_legacy_value(getattr(self, "build_version_tag", None), serialize),
            ("ok" if serialize else "ok"): _to_legacy_value(getattr(self, "ok", None), serialize),
            ("serverName" if serialize else "server_name"): _to_legacy_value(getattr(self, "server_name", None), serialize),
        }
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "buildVersionTag": obj.get("buildVersionTag"),
            "ok": obj.get("ok"),
