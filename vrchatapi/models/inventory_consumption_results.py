...
class InventoryConsumptionResults(BaseModel):
    """
    ...
    inventory_items: Optional[List[InventoryItem]] = Field(default=None, validation_alias=AliasChoices("inventoryItems", "inventory_items"), serialization_alias="inventoryItems")
    inventory_items_created: Optional[StrictInt] = Field(default=None, validation_alias=AliasChoices("inventoryItemsCreated", "inventory_items_created"), serialization_alias="inventoryItemsCreated")
    ...
    __properties: ClassVar[List[str]] = ["errors", "inventoryItems", "inventoryItemsCreated"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "inventoryItems" not in obj and "inventory_items" in obj:
            obj["inventoryItems"] = obj["inventory_items"]
        obj.pop("inventory_items", None)
        if "inventoryItemsCreated" not in obj and "inventory_items_created" in obj:
            obj["inventoryItemsCreated"] = obj["inventory_items_created"]
        obj.pop("inventory_items_created", None)
        return obj
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("errors" if serialize else "errors"): _to_legacy_value(getattr(self, "errors", None), serialize),
            ("inventoryItems" if serialize else "inventory_items"): _to_legacy_value(getattr(self, "inventory_items", None), serialize),
            ("inventoryItemsCreated" if serialize else "inventory_items_created"): _to_legacy_value(getattr(self, "inventory_items_created", None), serialize),
        }
        ...
        if self.inventory_items:
            for _item_inventory_items in self.inventory_items:
                if _item_inventory_items:
                    _items.append(_to_openapi_value(_item_inventory_items))
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "inventoryItemsCreated": obj.get("inventoryItemsCreated")
        })
        return _obj
