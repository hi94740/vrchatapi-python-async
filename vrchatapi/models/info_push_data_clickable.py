...
class InfoPushDataClickable(BaseModel):
    """
    ...
    """ # noqa: E501
    parameters: Optional[List[StrictStr]] = Field(default=None, description="In case of OpenURL, this would contain the link.")
    command: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(default=None, json_schema_extra={"examples": ["OpenURL"]})
    ...
    __properties: ClassVar[List[str]] = ["parameters", "command"]

    @field_validator('command')
    def command_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CannedWorldSearch', 'OpenAccountUpgrade', 'OpenAvatarsMenu', 'OpenHelpArticle', 'OpenListingDetails', 'OpenSafetyMenu', 'OpenURL', 'OpenVRCPlusMenu', 'OpenVRChatStore', 'OpenWorldDetails', 'OpenWorldsMenu', 'scrollToShelf']):
            raise ValueError("must be one of enum values ('CannedWorldSearch', 'OpenAccountUpgrade', 'OpenAvatarsMenu', 'OpenHelpArticle', 'OpenListingDetails', 'OpenSafetyMenu', 'OpenURL', 'OpenVRCPlusMenu', 'OpenVRChatStore', 'OpenWorldDetails', 'OpenWorldsMenu', 'scrollToShelf')")
        return value
