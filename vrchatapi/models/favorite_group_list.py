...
class FavoriteGroupList(BaseModel):
    """
    ...
    """ # noqa: E501
    favorite_groups: Optional[List[FavoriteGroupSummary]] = Field(default=None, validation_alias=AliasChoices("favoriteGroups", "favorite_groups"), serialization_alias="favoriteGroups")
    max_favorite_groups: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, validation_alias=AliasChoices("maxFavoriteGroups", "max_favorite_groups"), serialization_alias="maxFavoriteGroups", description="Only returned when the owner is the currently authenticated user.")
    max_favorites_per_group: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=None, validation_alias=AliasChoices("maxFavoritesPerGroup", "max_favorites_per_group"), serialization_alias="maxFavoritesPerGroup", description="Only returned when the owner is the currently authenticated user.")
    ...
    __properties: ClassVar[List[str]] = ["favoriteGroups", "maxFavoriteGroups", "maxFavoritesPerGroup"]
    ...
    def __preprocess_input_names(
        ...
        if not isinstance(obj, _Mapping):
            return obj
        obj = dict(obj)
        if "favoriteGroups" not in obj and "favorite_groups" in obj:
            obj["favoriteGroups"] = obj["favorite_groups"]
        obj.pop("favorite_groups", None)
    ...
    def to_dict(self, serialize: bool = False) -> Dict[str, Any]:
        ...
        return {
            ("favoriteGroups" if serialize else "favorite_groups"): _to_legacy_value(getattr(self, "favorite_groups", None), serialize),
    ...
    def __openapi_generator_modern_projection(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in favorite_groups (list)
        _items = []
        if self.favorite_groups:
            for _item_favorite_groups in self.favorite_groups:
                if _item_favorite_groups:
                    _items.append(_to_openapi_value(_item_favorite_groups))
            _dict['favoriteGroups'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "favoriteGroups": [FavoriteGroupSummary.from_dict(_item) for _item in obj["favoriteGroups"]] if obj.get("favoriteGroups") is not None else None,
            "maxFavoriteGroups": obj.get("maxFavoriteGroups"),
            "maxFavoritesPerGroup": obj.get("maxFavoritesPerGroup")
        })
        return _obj
