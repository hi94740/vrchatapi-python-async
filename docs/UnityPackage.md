# UnityPackage



## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**asset_url** | **str** |  | [optional] 
**asset_url_object** | **object** |  | [optional] 
**asset_version** | **int** |  | 
**created_at** | **datetime** |  | [optional] 
**id** | **str** |  | 
**impostor_url** | **str** |  | [optional] 
**impostorizer_version** | **str** |  | [optional] 
**performance_rating** | [**PerformanceRatings**](PerformanceRatings.md) |  | [optional] 
**platform** | **str** | This is normally &#x60;android&#x60;, &#x60;ios&#x60;, &#x60;standalonewindows&#x60;, &#x60;web&#x60;, or the empty value &#x60;&#x60;, but also supposedly can be any random Unity version such as &#x60;2019.2.4-801-Release&#x60; or &#x60;2019.2.2-772-Release&#x60; or even &#x60;unknownplatform&#x60;. | 
**plugin_url** | **str** |  | [optional] 
**plugin_url_object** | **object** |  | [optional] 
**scan_status** | **str** |  | [optional] 
**unity_sort_number** | **int** |  | [optional] 
**unity_version** | **str** |  | [default to '5.3.4p1']
**variant** | **str** |  | [optional] 
**world_signature** | **str** |  | [optional] 

## Example

```python
from vrchatapi.models.unity_package import UnityPackage

# TODO update the JSON string below
json = "{}"
# create an instance of UnityPackage from a JSON string
unity_package_instance = UnityPackage.from_json(json)
# print the JSON string representation of the object
print(UnityPackage.to_json())

# convert the object into a dict
unity_package_dict = unity_package_instance.to_dict()
# create an instance of UnityPackage from a dict
unity_package_from_dict = UnityPackage.from_dict(unity_package_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


