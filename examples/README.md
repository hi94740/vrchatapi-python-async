...
All API methods are asynchronous in this SDK, so every call must be `await`ed
and the `ApiClient` is used as an async context manager (`async with`).
...
async def main():
    ...
    async with vrchatapi.ApiClient(configuration) as api_client:
        ...
        active_worlds = await worlds_api.get_active_worlds()
